...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".mimo-project-id")
  const idFile = nodePath.join(mainGit, "mimocode-project-id")

  const localExists = await nodeFs.access(localFile).then(() => true, () => false)
  if (localExists) {
    const idExists = await nodeFs.access(idFile).then(() => true, () => false)
    if (!idExists) {
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .mimo-project-id
  const excludeFile = nodePath.join(mainGit, "info", "exclude")
  await nodeFs.mkdir(nodePath.dirname(excludeFile), { recursive: true })
  const existing = await nodeFs.readFile(excludeFile, "utf-8")
    .catch(() => "")
  if (!existing.includes(".mimo-project-id")) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.mimo_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      if (Flag.mimo_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
