...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const init = Effect.fn("Command.state")(function* (ctx: InstanceContext) {
      ...
      commands[Default.SKILL_AUTO] = {
        name: Default.SKILL_AUTO,
        description: "AI-driven skill selection: analyze the request and auto-inject the best skills for the worker",
        source: "command",
        subtask: false,
        get template() {
          return [
            "You are a skill-routing assistant. Your job is to analyze the user's request",
            "and select the optimal set of skills to inject into the main AI worker.",
            "",
            "User request:",
            "$ARGUMENTS",
            "",
            "Steps:",
            "1. Read and analyze the user's request above.",
            "2. List ALL available skills by calling the skill tool or reading the available skills block.",
            "3. For each skill, evaluate if it's relevant to the request based on:",
            "   - Skill description matching the task domain",
            "   - Skill content containing useful patterns for this type of work",
            "   - Whether the skill would help the worker avoid reinventing patterns",
            "4. Select the TOP skills (max 5) that would be most beneficial.",
            ...
            "    <skill name=\"...\" reason=\"...\" />",
            "  </selected>",
            "  <analysis>Brief analysis of why these skills were chosen</analysis>",
            "</skill:auto-selection>",
            "",
            "The main worker will receive the selected skills injected into its context.",
            "Be selective - only recommend skills that genuinely help with this specific task.",
          ].join("\n")
        },
        hints: ["$ARGUMENTS"],
      }
