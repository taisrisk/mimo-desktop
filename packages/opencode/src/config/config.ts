  ...
  Effect.gen(function* () {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (Flag.mimo_CONFIG) {
          yield* merge(Flag.mimo_CONFIG, yield* loadFile(Flag.mimo_CONFIG))
        ...
        if (Flag.mimo_CONFIG_DIR) {
          log.debug("loading config from MIMOCODE_CONFIG_DIR", { path: Flag.mimo_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .mimo commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".mimo") || dir === Flag.mimo_CONFIG_DIR) {
        ...
        if (process.env.mimo_CONFIG_CONTENT) {
          const source = "MIMOCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.mimo_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
        ...
        if (!Flag.mimo_DISABLE_CLAUDE_CODE_MCP) {
