...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  // Every config dir we may read from: global config dir, any `.mimo`
  // folders between cwd and home, and MIMOCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.mimo_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  if (Flag.mimo_TUI_CONFIG) {
    const configFile = Flag.mimo_TUI_CONFIG
  ...
  // 4. `.mimo` directories (and MIMOCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".mimo") || dir === Flag.mimo_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".mimo") && dir !== Flag.mimo_CONFIG_DIR) continue
