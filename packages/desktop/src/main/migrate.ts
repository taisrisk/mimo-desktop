...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.mimo.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `mimo.settings.dat` is special: it maps to the `mimo.settings` store
...
function migrateFile(datPath: string, filename: string) {
  ...
  // mimo.settings.dat → the electron settings store ("mimo.settings").
  ...
  const storeName = filename === "mimo.settings.dat" ? "mimo.settings" : filename
