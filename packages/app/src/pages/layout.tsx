...
import { MimoHub } from "@/components/mimo-hub"
...
export default function Layout(props: ParentProps) {
  ...
  const [mimoHubOpen, setMimoHubOpen] = createSignal(false)

  const MIMO_COMMAND_TO_TAB: Record<string, "memory" | "agents" | "goals" | "compose" | "dream"> = {
    "mimo.memory.toggle": "memory",
    "mimo.subagents.toggle": "agents",
    "mimo.goals.view": "goals",
    "mimo.compose.toggle": "compose",
    "mimo.dream.start": "dream",
  }

  onMount(() => {
    makeEventListener(window, "mimo:command", (e: Event) => {
      const detail = (e as CustomEvent<{ command: string }>).detail
      if (!detail?.command) return
      const tab = MIMO_COMMAND_TO_TAB[detail.command]
      if (tab !== undefined) setMimoHubOpen(true)
    })
  })
  ...
  command.register("layout", () => {
    const commands: CommandOption[] = [
      {
        id: "mimo.hub.toggle",
        title: "Toggle Mimo Hub",
        category: "Mimo",
        keybind: "mod+shift+m",
        onSelect: () => setMimoHubOpen((v: boolean) => !v),
      },
      {
        id: "sidebar.toggle",
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("https://github.com/taisrisk/mimo-desktop/issues")}
      renderPanel={() =>
  ...
  return (
    <div class="relative flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text" style="background:var(--helios-bg)">
      ...
      <div class="flex-1 min-h-0 min-w-0 flex">
        ...
        <Show when={mimoHubOpen()}>
          <MimoHub onClose={() => setMimoHubOpen(false)} />
        </Show>
