...
function getProjectGradient(name: string) {
  const idx = hashString(name) % GRADIENT_PALETTES.length
...
export const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
  ...
  const gradient = createMemo(() => {
    if (props.project.icon?.url || props.project.icon?.override) return null
    return getProjectGradient(name())
  })
    ...
    <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
      <div class="size-full rounded overflow-clip">
        ...
        >
          {(grad) => (
            <div
              class="size-full rounded flex items-center justify-center text-white font-bold text-xs"
              style={{
