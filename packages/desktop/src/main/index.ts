...
const APP_IDS: Record<string, string> = {
  dev: "ai.mimo.desktop.dev",
  beta: "ai.mimo.desktop.beta",
  prod: "ai.mimo.desktop",
}
const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.mimo.desktop.dev"
...
import type { Server } from "virtual:mimo-server"
...
async function initialize() {
  ...
  const loadingTask = (async () => {
    ...
    if (needsMigration) {
      const { Database, JsonMigration } = await import("virtual:mimo-server")
...
function wireMenu() {
  ...
  // Keep window recovery relaunch in sync with menu relaunch (kills sidecar first)
  setRelaunchHandler(doRelaunch)
