...
const EXTERNAL_DIRS = [".claude", ".agents", ".codex", ".mimo", ".gemini", ".cursor", ".hermes", ".antigravity", ".openclaw"]
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.mimo_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.mimo_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.mimo_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.mimo_DISABLE_CODEX_SKILLS) return false
      if (dir === ".mimo" && Flag.mimo_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
