...
async function ensureCheckpointTemplate(checkpointFile: string): Promise<void> {
  const checkpointExists = await fs.access(checkpointFile).then(() => true, () => false)
  if (!checkpointExists) {
    await fs.writeFile(checkpointFile, CHECKPOINT_TEMPLATE)
  }
  const memoryExists = await fs.access(memoryFile).then(() => true, () => false)
  if (!memoryExists) {
    await fs.writeFile(memoryFile, MEMORY_TEMPLATE)
  }
  const notesExists = await fs.access(notesFile).then(() => true, () => false)
  if (!notesExists) {
...
async function ensureMemoryTemplate(memoryFile: string): Promise<void> {
  const exists = await fs.access(memoryFile).then(() => true, () => false)
  if (!exists) {
...
async function ensureNotesTemplate(notesFile: string): Promise<void> {
  const exists = await fs.access(notesFile).then(() => true, () => false)
  if (!exists) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const tryStartCheckpointWriter: (
      ...
      const checkpointExists = yield* Effect.promise(() => fs.access(checkpointFile).then(() => true, () => false))
      const memoryExists = yield* Effect.promise(() => fs.access(memoryFile).then(() => true, () => false))
    ...
    const hasCheckpoint = Effect.fn("SessionCheckpoint.hasCheckpoint")(function* (sessionID: SessionID) {
      return yield* Effect.promise(() => fs.access(checkpointPath(sessionID)).then(() => true, () => false))
    })
    ...
    const renderIndex = Effect.fn("SessionCheckpoint.renderIndex")(function* (sessionID: SessionID) {
      ...
      const exists = yield* Effect.promise(() => fs.access(snapFile).then(() => true, () => false))
